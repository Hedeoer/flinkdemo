...
/**
 * tumblingwindowjoin 的sql实现
 * 1. 为两张表都应用一个表值函数 TUMBLE
 * 2. 使用inner join，条件为key相同，并且滚动窗口相同
 *
 * 同理： 对于datastream 的 slidingWindowJoin的效果 在flink sql中可以使用HOP的表值函数实现，类似于下面对tumblingwindowjoin的sql实现
 */
public class $02TumblingWindowJoinBySQL {
    public static void main(String[] args) throws Exception {
        ...
        * */
        ...
        TableResult result = tableEnvironment.executeSql("SELECT\n" +
                ...
                "    ) AS join_result\n" +
                "FROM\n" +
                "    TABLE(TUMBLE(TABLE green_source, DESCRIPTOR(event_time), INTERVAL '5' SECONDS)) AS g\n" +
                "JOIN\n" +
                "    TABLE(TUMBLE(TABLE orange_source, DESCRIPTOR(event_time), INTERVAL '5' SECONDS)) AS o\n" +
                "ON\n" +
                "    g.`key` = o.`key`\n" +
                "AND\n" +
