...
public class DataGenerator {
    ...
    public short passengerCnt() {
        passengerCnt = (short) aLong(1L, 7L);
        return passengerCnt;
    }
    ...
    // 模拟行程的所载的乘客名单，数组存储.返回值是数组

    // 模拟乘客名单
    public String[] getPassengerNames(){
        // 候选乘客
        String[] passengerNamesCadidates = {"Alice", "Bob", "Charlie", "David", "Edith", "Frank", "George", "Helen", "Irene", "James", "Kevin", "Laura", "Maria", "Nancy", "Oliver", "Peter", "Quinn", "Rachel", "Sarah", "Tim", "Ursula", "Victor","Wendy", "Xavier", "Yvonne"};
        // 乘客数量
        short nums = passengerCnt;
//        System.out.println("候选乘客数量" + passengerNamesCadidates.length);
//        System.out.println("模拟乘客数量" + nums);
        // 乘客起始索引
        Set<String> selectedNames = new HashSet<>();
        Random random = new Random();

        while (selectedNames.size() < nums) {
            int index = random.nextInt(passengerNamesCadidates.length);
            selectedNames.add(passengerNamesCadidates[index]);
        }
    ...
    public String currency() {
        String[] currencyNames = {"USD", "EUR", "JPY"};
